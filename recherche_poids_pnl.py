...
def calcul_sortie (liste_entree: list[float],liste_poids: list[float]):
    sorties=0
    for i in range (len (liste_entree)):
        sorties+=liste_poids[i]*liste_entree [i]
    return sorties
...
def descente_gradient_stochastique (liste_entree: list[list[int]],liste_sortie:list[int],liste_poid:list[float],n:float):
    ...
    while corrections >e:
        ...
        for i in range(len(liste_entree)):
            ...
            if d !=0:
                liste_poids[0]=liste_poids[0]-n*d*sortie*(1-sortie)
                corrections=corrections+abs(n*d*sortie*(1-sortie))
                for j in range (len (liste_entree)):
