...
def DGS(patience=10):
    ...
    On itère tant que E_test diminue. Si E_test ne diminue pas,
    on attend encore 'patience' itérations avant de s'arrêter.
    
    Paramètre  : patience (int) — nombre d'itérations supplémentaires
                 à faire si E_test ne diminue pas. Défaut = 10.
    Retourne   : rien. Affiche le graphique des droites.
    Précondition : percep, X_train, Y_train, X_test, Y_test initialisés.
    """
    E_test_min   = calcule_erreur_test()  # meilleur E_test vu jusqu'ici
    patience_cpt = 0                      # compteur d'itérations sans amélioration
    iteration    = 0

    while patience_cpt <= patience:
        iteration += 1

        # Mélanger les exemples à chaque itération
        permutation_train = gnpa.permutation(np.arange(N_train))

        # Correction des poids sur X_train
        for i in range(N_train):
            idx = permutation_train[i]
        ...
        print(f"Itération {iteration} — E_test = {E_test_actuel:.4f} — patience = {patience_cpt}/{patience}")
        ajoute_droite(ax, percep[0], percep[1], percep[2])

        # Mise à jour du compteur de patience
        if E_test_actuel < E_test_min:
            E_test_min   = E_test_actuel  # amélioration → on remet le compteur à 0
            patience_cpt = 0
        else:
            patience_cpt += 1             # pas d'amélioration → on incrémente

    ajoute_dernière_droite(fig, ax, percep[0], percep[1], percep[2])
    print(f"\nArrêt après {iteration} itérations.")
    print(percep[0],percep[1],percep[2])
